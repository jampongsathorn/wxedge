...
**รอบการทำงาน (v5 · โซ่ต่อเนื่อง):**
- **โซ่**: job เดียววนทุก 10 นาที นาน ~5.5 ชม. แล้ว dispatch ตัวเองต่อ (cron */30 = ตัวสำรอง) → `deploy/chain_loop.sh`
- แต่ละรอบใช้ `deploy/gate.py` เลือกเมืองตาม 2 ช่วงเวลา: **entry 15:00–16:30 local ทุก 5 นาที** และ **ctx 12:30–18:30 ทุก 15 นาที**
- ⚠ เกณฑ์เว้นระยะ (`ENTRY_GAP`/`CTX_GAP`) **ต้องน้อยกว่ารอบตื่น** (`INTERVAL_MIN`) ไม่งั้นถูกข้ามเป็นรอบคู่ (บทเรียน 25 ก.ย.: รอบตื่นห่างจริง 4 น. 45 วิ) — มีเทสต์ `tests_chain.py` บังคับไว้
  + เว้นระยะต่อเมืองโดยอ่านสถานะจาก `data/snapshots/<วัน>.jsonl` · รอบที่ไม่มีเมือง = จบ ~15 วิ
- รอบที่มีเมืองจริง → สแกนเฉพาะเมืองนั้น (`--workers 4`) ~40–90 วิ
- ⚠️ ต้องเป็น **repo สาธารณะ**: โซ่ที่รันต่อเนื่อง 24 ชม./วัน = 43,200 นาที/เดือน เกินโควตาฟรี 2,000 ของ repo ส่วนตัวหลายเท่า
- รายวัน 03:20 UTC → จักรวาล + bid/ask 90 bins + `forward_resolve.py` เติมเฉลย + รายงาน hit rate จริง
- นาที CI (repo สาธารณะ = ไม่คิดนาที · repo ส่วนตัวต้องลด cadence ลงเหลือ ~30 นาทีเพื่อให้อยู่ใน 2,000)
